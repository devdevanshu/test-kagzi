...
@section('content')
    <div class="p-6">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-600 mt-2">View and manage archived customer inquiries</p>
                </div>
                <a href="{{ route('contacts.index') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600">Archived Today</p>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6 border border-gray-200">
            <form method="GET" action="{{ route('contacts.archived') }}" class="flex flex-col md:flex-row gap-4 items-end justify-between">
                <div class="flex-1 w-full md:w-auto">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Search Archived Messages</label>
                    <input type="text" 
                           name="search"
                ...
                <div class="flex gap-2 w-full md:w-auto">
                    ...
                    <a href="{{ route('contacts.archived') }}" class="flex-1 md:flex-none inline-flex items-center px-4 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-300 transition-colors">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 transition-all duration-300 hover:shadow-md border border-gray-200 border-l-4 border-l-gray-600">
                <div class="flex flex-col md:flex-row gap-6">
                    <!-- Contact Info -->
                    <div class="flex-1">
                        <div class="flex items-start gap-4 mb-4">
                            <div class="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold bg-gray-600">
                                {{ substr($contact->name ?? 'U', 0, 1) }}
                            </div>
                            <div class="flex-1">
                                <h3 class="text-lg font-bold mb-1 text-gray-900">
                                    {{ $contact->name ?? 'Anonymous' }}
                                    <span class="inline-flex px-2 py-1 text-xs font-medium bg-gray-100 text-gray-800 rounded-full ml-2">Archived</span>
                                </h3>
                        ...
                        <div class="p-4 rounded-lg bg-gray-50">
                            <p class="text-sm font-medium mb-2 text-gray-600">Message:</p>
                    ...
                    <div class="md:w-64 flex flex-col gap-4">
                        ...
                        <div class="flex flex-col gap-2">
                            <button onclick="unarchiveMessage({{ $contact->id }})" class="w-full px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 transition-colors">
            ...
            <div class="bg-white rounded-lg shadow-sm p-12 text-center border border-gray-200">
                ...
                <h3 class="text-xl font-bold mb-2 text-gray-900">No Archived Messages</h3>
                <p class="text-gray-600">Archived messages will appear here when you archive them from the main messages page.</p>
            </div>
            @endforelse
        </div>
    </div>
    ...
    <script>
        ...
        function unarchiveMessage(contactId) {
            if (confirm('Are you sure you want to restore this message?')) {
                fetch(`/contacts/${contactId}/unarchive`, {
                    method: 'POST',
                    headers: {
