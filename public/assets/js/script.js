(function($) {
	...
	if (typeof Lenis !== 'undefined') {
		try {
			...
			lenis.on('scroll', () => {
				// Scroll callback can be added here
			})
  ...
  // Initialize ScrollTrigger only if it's available
   ...
   ========================================================================== */
	...
	$(document).ready(function() {
		...
		$('a[href^="#"]').on('click', function(e) {
			...
			// Don't prevent default if it's an empty hash
			if (target === '#' || target === '') {
				return;
			}
			...
			if (targetElement.length) {
				...
				$('html, body').animate({
					scrollTop: targetElement.offset().top - 80 // 80px offset for header
				}, 800, 'swing'); // Using built-in swing easing
